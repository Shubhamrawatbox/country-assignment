...
const ItemDetail = () => {
  ...
  return (
    <>
      <button onClick={()=>navigate(-1)} className={`back_btn ${initialTheme === 'dark' ? 'dark' : 'light'}`}>
        <IoMdArrowRoundBack />
        Back
      </button>
      <div className="details-container">
        ...
        <div className="rightSideContent">
          ...
          <div className="infoGridList">
            ...
            <InfoTitle newkey={"Currencies"} value={getCountryCurrency(currencies)} />
          ...
          <div className="border_name_container">
            ...
            <div style={{ display: "flex", flexWrap: "wrap" }}>
              {borders?.map((borderName:any, index: number) => (
                <div key={index} className={`border_name_box ${initialTheme === 'dark' ? 'dark' : 'light'}`}>
                  {borderName}
                </div>
              ))}
            </div>
